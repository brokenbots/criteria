...
- **`working_directory`** (optional): Launch directory for the adapter process. Shell and copilot adapters bound to the environment run in this directory by default (it becomes the process cwd). Folded at compile time, so it may reference declared variables and locals (e.g. `working_directory = var.worktree`); runtime-only references (`each.value`, `steps.X.outputs.Y`) are rejected. Accepted by `shell`, `sandbox`, and `remote` environments; **not** accepted by `container` environments, which isolate paths rather than relocate the process cwd. For `sandbox` environments the path must also be permitted by the filesystem policy so the chdir succeeds inside the sandbox.
