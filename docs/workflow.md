...
`--events-file <path>` (write the ND-JSON event stream to a file; in `--server` mode the file mirrors the server stream so both carry the same envelope content, including `adapter.lifecycle` events with `scope_instance_id`, `environment_type`, `environment_name`, `shim_listen_address`, `token_ref`, and the wire-delivered `accept_token`),
