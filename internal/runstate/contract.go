...
//	Agent         {criteriaId, name, status, lastSeenAt?}
//	EventEnvelope {schemaVersion, runId, seq, type, correlationId, payload}
//	RunEventsPage {events: EventEnvelope[], nextPageToken}
//	RunInspection {sessionId, adapter?, currentStep, pendingPermissions,
//	               lastActivityAt?, stateJson?}
//
...
// event vocabulary is identical to the castle path: each NDJSON line is a
// plain-JSON rendering of a criteria.v1 Envelope, so type is the payload
// discriminator (the ND-JSON payload_type value) and payload is the raw
...
// contract-acceptable for the local seam (CRI-279).
type Agent struct {
	CriteriaID string `json:"criteriaId,omitempty"`
...
// RunEventsPage is the paginated run-events response.
