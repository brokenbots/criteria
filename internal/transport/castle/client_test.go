...
type fakeServer struct {
	...
	// simulates the persist-before-ack reconnect window (Castle wrote,
...
func (f *fakeServer) Register(_ context.Context, _ *connect.Request[pb.RegisterRequest]) (*connect.Response[pb.RegisterResponse], error) {
	return connect.NewResponse(&pb.RegisterResponse{OverseerId: f.overseerID, Token: f.token}), nil
}
...
func (f *fakeServer) CreateRun(_ context.Context, req *connect.Request[pb.CreateRunRequest]) (*connect.Response[pb.Run], error) {
	...
	r := &pb.Run{RunId: id, OverseerId: req.Msg.OverseerId, WorkflowName: req.Msg.WorkflowName, Status: "pending"}
...
func (f *fakeServer) SubmitEvents(ctx context.Context, stream *connect.BidiStream[pb.Envelope, pb.Ack]) error {
	...
	for {
		...
		// ack don't double-persist. Mirrors Castle's real behavior.
		var seq uint64
		duplicate := false
...
func TestClientHappyPath(t *testing.T) {
	...
	if c.OverseerID() != "ovr-1" {
		t.Fatalf("overseer id: %q", c.OverseerID())
	}
...
// TestClientPersistBeforeAckReconnect simulates the window where Castle has
// persisted an envelope but the connection drops before the ack is delivered.
// On reconnect, Castle replays the ack from its prior persisted events.
