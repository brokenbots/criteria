...
//  1. Spins up a fake Castle Connect server that returns a VariableScope JSON
...
//
// This exercises the full Castle ReattachRun → RestoreVarScope → resumed
...
func TestReattachRun_RestoresVarScope(t *testing.T) {
	...
	// Build a Castle client (no auth needed for h2c test server).
	...
	resp, err := client.ReattachRun(ctx, "run-1", srv.overseerID)
	if err != nil {
