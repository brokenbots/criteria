...
func compileToMap(t *testing.T, path string) map[string]any {
	t.Helper()
	b, err := compileWorkflowOutput(context.Background(), path, "json", nil, false, false)
	if err != nil {
...
`
	b, err := compileWorkflowOutput(context.Background(), writeParent(t, dir, hcl), "json", nil, false, false)
	if err != nil {
		t.Fatalf("compileWorkflowOutput: %v", err)
	}
