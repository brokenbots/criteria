...
func TestDiscoverBinaryPrefersEnvOverHome(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(homeDir, ".overlord", "plugins"), 0o755); err != nil {
		t.Fatalf("mkdir home plugins: %v", err)
	}
	envPath := filepath.Join(envDir, "overlord-adapter-noop")
	homePath := filepath.Join(homeDir, ".overlord", "plugins", "overlord-adapter-noop")
	writeExecutable(t, envPath)
	writeExecutable(t, homePath)

	t.Setenv("OVERLORD_PLUGINS", envDir)
...
func TestDiscoverBinaryFallsBackToHome(t *testing.T) {
	...
	t.Setenv("OVERLORD_PLUGINS", filepath.Join(t.TempDir(), "missing"))
	t.Setenv("HOME", homeDir)
...
func TestDiscoverBinaryNotFoundIncludesSearchedPaths(t *testing.T) {
	...
	want := []string{
		filepath.Join(envDir, "overlord-adapter-copilot"),
		filepath.Join(homeDir, ".overlord", "plugins", "overlord-adapter-copilot"),
	}
