...
func runWorkflowBody(ctx context.Context, body *workflow.FSMGraph, bodyEntry string, childVars map[string]cty.Value, workflowDir string, deps Deps, rlc *remoteLifecycleContext, scopeName string, parallelCeiling int, parallelSem chan struct{}, parallelSemCache map[parallelSemKey]chan struct{}, parallelSemMu *sync.Mutex, crashedCommentSessions, crashedFunctionalSessions *crashedSessionRefs, failedSteps *failedStepTracker, ancestors ...string) (terminal string, returnOutputs, finalVars map[string]cty.Value, err error) {
	...
	bodyOrder, childSt, err := startWorkflowBody(ctx, body, bodyEntry, childVars, workflowDir, deps, rlc, scopeName, parallelCeiling, parallelSem, parallelSemCache, parallelSemMu, crashedCommentSessions, crashedFunctionalSessions, failedSteps, ancestors)
	if err != nil {
		return "", nil, nil, err
	}
...
func startWorkflowBody(ctx context.Context, body *workflow.FSMGraph, bodyEntry string, childVars map[string]cty.Value, workflowDir string, deps Deps, rlc *remoteLifecycleContext, scopeName string, parallelCeiling int, parallelSem chan struct{}, parallelSemCache map[parallelSemKey]chan struct{}, parallelSemMu *sync.Mutex, crashedCommentSessions, crashedFunctionalSessions *crashedSessionRefs, failedSteps *failedStepTracker, ancestors []string) ([]string, *RunState, error) {
	...
	childSt := &RunState{
		...
		// CRI-274: subworkflow bodies share the caller's failed-step tracker so
		// a failure recorded in the callee taints the whole run's completion.
