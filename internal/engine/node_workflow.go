...
func runWorkflowBody(ctx context.Context, body *workflow.FSMGraph, bodyEntry string, childVars map[string]cty.Value, workflowDir string, deps Deps, rlc *remoteLifecycleContext, scopeName string, parallelCeiling int, parallelSem chan struct{}, parallelSemCache map[parallelSemKey]chan struct{}, parallelSemMu *sync.Mutex, crashedCommentSessions *crashedSessionRefs, ancestors ...string) (terminal string, returnOutputs, finalVars map[string]cty.Value, err error) {
	...
	bodyOrder, childSt, err := startWorkflowBody(ctx, body, bodyEntry, childVars, workflowDir, deps, rlc, scopeName, parallelCeiling, parallelSem, parallelSemCache, parallelSemMu, crashedCommentSessions, ancestors)
	if err != nil {
		return "", nil, nil, err
	}
...
func startWorkflowBody(ctx context.Context, body *workflow.FSMGraph, bodyEntry string, childVars map[string]cty.Value, workflowDir string, deps Deps, rlc *remoteLifecycleContext, scopeName string, parallelCeiling int, parallelSem chan struct{}, parallelSemCache map[parallelSemKey]chan struct{}, parallelSemMu *sync.Mutex, crashedCommentSessions *crashedSessionRefs, ancestors []string) ([]string, *RunState, error) {
