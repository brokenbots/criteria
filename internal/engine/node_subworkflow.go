...
func runSubworkflow(ctx context.Context, stepName string, node *workflow.SubworkflowNode, parentSt *RunState, stepInput map[string]cty.Value, deps Deps, ancestors ...string) (outputs map[string]cty.Value, terminal string, err error) {
	...
	terminal, returnOutputs, finalVars, err := runWorkflowBody(ctx, node.Body, node.BodyEntry, childVars, calleeDir, deps, parentSt.RemoteLifecycle, stepName, parentSt.ParallelCeiling, parentSt.ParallelSem, parentSt.ParallelSemCache, parentSt.ParallelSemMu, parentSt.CrashedCommentSessions, parentSt.CrashedFunctionalSessions, parentSt.FailedSteps, childAncestors...)
	if err != nil {
