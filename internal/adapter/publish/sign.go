...
//
// Key mode (KeySigner) returns empty cert/chain. A future keyless signer
// (Sigstore OIDC, CI only) returns the Fulcio certificate + chain PEM, which
// stageSignature attaches as the dev.sigstore.cosign/{certificate,chain}
// annotations the verifier reads for keyless identities.
