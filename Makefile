...
test-cover: ## Run tests with coverage and print per-package summaries (no -race to keep it fast)
	go test -coverprofile=cover.out -covermode=atomic ./...
	cd sdk      && go test -coverprofile=cover-sdk.out -covermode=atomic ./...
	cd workflow && go test -coverprofile=cover-workflow.out -covermode=atomic ./...
...
bench: ## Run all benchmarks (workflow compile, engine, plugin)
	go test -run='^$$' -bench=. -benchmem -benchtime=3s ./workflow/...
	go test -run='^$$' -bench=. -benchmem -benchtime=3s ./internal/engine/...
	go test -run='^$$' -bench=. -benchmem -benchtime=3s ./internal/plugin/...
